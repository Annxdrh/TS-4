...
class User implements IUser{
    ...
    signUp(login: string, password:string,rptPassword:string){
        //login id in users?
        //min password length 15
        //login valodate: Lladjdssdfk_-!@#$%&*() => re
        if(login.length>12){
            if(password.length>12){
                return{"status":"403","text":"Forbidden","message":"Пароли не совпадают"}
            }
